...
var GwcCommon = {
	...
	toRegionalUrl: function(urlPath) {
		...
		var selectedLang = _.find(regionalData.langs, function(data) {return data.isCurrent;});
		if (!!selectedLang) {
...
jQuery(function($){
	...
	function gheadermodalmenuOpen(_target){
		...
		$('#lightbox-overlay').append('<div id="modalmenu-box-wrapper"><div id="modalmenu-box-bg"><div id="modalmenu-box"></div><a href="#" class="btn-close"><img src="https://www.panasonic.com/etc/designs/panasonic/common-clientlibs/images/btn-close.png" alt="Close"></a></div></div>');
	...
	$('#globalheader .toggleMenu').attr('aria-expanded','false').find('.label').filter(function(index) {
		if($('link[href*="header-white"]').length){
			$(this).html('<img src="https://www.panasonic.com/etc/designs/panasonic/common-clientlibs/images/icn-nav-global-menu-bk-m-2x.png" alt="'+$(this).text()+'">');
		} else {
			$(this).html('<img src="https://www.panasonic.com/etc/designs/panasonic/common-clientlibs/images/icn-nav-global-menu-m-2x.png" alt="'+$(this).text()+'">');
		}
	});
	...
	$("#nav-menu-mob .accordion > li a.accordion-head").each(function(index, element) {
		...
		$(this).click(function(e) {
			...
			if ($.isFunction($.fn.imageConverter)) {
				$('> li > a .img-element img[data-type="delay-load"]', ul).filter(function() {
					return !$(this).data('ic-state');
				}).imageConverter();
			}
...
function picBoxOpen(_target,_alt,_title){
	overlayset('nonfix');
	jQuery(function($){
		$('#lightbox-overlay').append('<div id="pic-box-wrapper"><div id="pic-box-bg"><div id="pic-box"></div><a href="#" class="btn-close"><img src="https://www.panasonic.com/etc/designs/panasonic/common-clientlibs/images/btn-close.png" alt="Close"></a></div></div>');
		var alt = _alt || "";
		var title = _title || "";
...
;(function($){
	$.fn.copyguardImage = function(){
		...
		if($('#page').css('min-width')=='320px' && navigator.userAgent.match(/Android/i)){
			var dummy = new Image();
			dummy.src = "https://www.panasonic.com/etc/designs/panasonic/common-clientlibs/images/pixel.png";
...
jQuery(function($){
	...
	if($('link[href*="header-white"]').length){
		$button.find('span.label').append('<img src="https://www.panasonic.com/etc/designs/panasonic/searchautocomplete-clientlibs/images/icn-search@2x.png" alt="'+label+'" class="open"><img src="https://www.panasonic.com/etc/designs/panasonic/common-clientlibs/images/icn-search-close-bk-m.png" alt="'+closealt+'" class="close">');
	} else {
		$button.find('span.label').append('<img src="https://www.panasonic.com/etc/designs/panasonic/searchautocomplete-clientlibs/images/icn-search-w@2x.png" alt="'+label+'" class="open"><img src="https://www.panasonic.com/etc/designs/panasonic/searchautocomplete-clientlibs/images/icn-search-close-m.png" alt="'+closealt+'" class="close">');
	}
